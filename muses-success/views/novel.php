...
class Novel extends Controller {
        ...
        function view()
        {
                ...
                if (($novel_id != '' && $this->novels->novel_exists($novel_id) == true) || (intval($novel_id) != 0 && $this->novels->novel_exists_id($novel_id) == true))
                {
						...
						$page_data["facebook"]["book:author:profile:username"] = $novel["author_pen"];
						$split_name = explode(" ", $novel["author_pen"]);
						$name_count = count($split_name);
						
						$page_data["facebook"]["book:author:first_name"] = $split_name[0];
						if (isset($split_name[$name_count-1])) {
							$page_data["facebook"]["book:author:last_name"] = $split_name[$name_count-1];							
						}						
						$page_data["facebook"]["url"] = $novel["listing_url"];
						$page_data["facebook"]["description"] = character_limiter(strip_tags($novel["summary"]),140);
						$page_data['meta_description'] = character_limiter("By ".$novel["author_pen"]." - ".$novel["primary_genre"]." - ".strip_tags($novel["summary"])."", 140);
